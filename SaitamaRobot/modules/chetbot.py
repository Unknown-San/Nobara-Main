...
ARQ_API_KEY = "GASCYH-LOIFSC-IGWRRR-GDDRPS-ARQ"
...
async def hmm(_, message):
    ...
    if len(message.command) != 2:
        await message.reply_text(
            "I only have 2 commands `/chatbot on` and /chatbot `off only`"
        )
    ...
    if status == "ON" or status == "on" or status == "On":
        ...
        if not lol:
            await lel.edit("Nobara AI Already Activated In This Chat baka ka omae?")
            return
        await lel.edit(
            f"Nobara AI Successfully Added For Users In The Chat {message.chat.id}"
        )
...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        pro = response
        try:
            await nobara.send_chat_action(message.chat.id, "typing")
...
@nobara.on_message(
    filters.regex("Nobara|Nobara|nobara|nobara|nobara")
    & ~filters.bot
    & ~filters.via_bot
    & ~filters.forwarded
    & ~filters.reply
...
__help__ = """
** AI Chatbot **
Nobara AI 3.0 IS THE ONLY AI SYSTEM WHICH CAN DETECT & REPLY UPTO 200 LANGUAGES
